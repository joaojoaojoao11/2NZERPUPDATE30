...
const DebtorCollectionModule: React.FC<{ currentUser: User }> = ({ currentUser }) => {
   ...
   const handleManageClient = async (cliente: string) => {
      ...
      try {
         ...
         const filtered = arData.filter(t => {
            ...
            const categoria = (t.categoria || '').toUpperCase().trim();
            const isAcordoParcela = categoria === 'ACORDO COMERCIAL';

            if (t.cliente !== cliente) return false;

            // REGRA ESPECIAL: Parcelas de Acordo só aparecem na inadimplência se VENCIDAS
            if (isAcordoParcela) {
               if (t.saldo <= 0.01) return false;
   ...
   const { priorityList, upToDateList } = useMemo(() => {
      ...
      filteredDebtors.forEach(d => {
         ...
         // 1. Se tem Acordo (`emAcordo` > 0), a prioridade é definida EXCLUSIVAMENTE pelo atraso do acordo.
         //    (Ignora dívidas soltas que possam ter sobrado, assumindo que o acordo cobre a gestão)
         // 2. Se não tem Acordo, qualquer dívida vencida (`totalVencido`) torna prioridade.
         const hasAgreement = (d.emAcordo || 0) > 0.01;
         const hasAgreementArrears = (d.acordoAtrasado || 0) > 0.01;
         const hasRegularArrears = d.totalVencido > 0.01;

         const hasArrears = hasAgreement ? hasAgreementArrears : hasRegularArrears;
                        ...
                        {!activeQuickAction ? (
                           <div className="grid grid-cols-2 gap-3">
                              ...
                              <button
                                 ...
                                 className={`flex flex-col items-center justify-center h-28 p-4 rounded-[1.5rem] transition-all group active:scale-95 shadow-md border ${selectedForAgreement.length === 0 ? 'opacity-50 cursor-not-allowed bg-gray-100 text-gray-400 border-gray-200' :
                                    isNotaryRemovalMode
