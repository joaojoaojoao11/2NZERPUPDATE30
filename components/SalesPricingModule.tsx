...
const SalesPricingModule: React.FC<{ user: User }> = ({ user }) => {
  ...
  const [activeTab, setActiveTab] = useState<'SIMULATOR' | 'ENGINEERING'>(user.role === 'DIRETORIA' ? 'ENGINEERING' : 'SIMULATOR');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        {/* Sistema de Abas */}
        {isDiretoria && (
          <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 self-start md:self-end">
            <button
              onClick={() => setActiveTab('ENGINEERING')}
              className={`px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'ENGINEERING' ? 'bg-white text-blue-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
            >
              Engenharia de Preços
            </button>
            <button
              onClick={() => setActiveTab('SIMULATOR')}
              className={`px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'SIMULATOR' ? 'bg-white text-blue-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
            >
              Simulador de Venda
            </button>
          </div>
