          ...
          {users.map(u => (
            <div key={u.id || u.email} className={`p-6 bg-white rounded-3xl border border-slate-100 shadow-sm hover:border-blue-200 transition-all ${!u.active ? 'opacity-60 bg-slate-50' : ''}`}>
              ...
              <div className="flex justify-between items-center mb-6">
                <span className={`px-3 py-1 rounded-xl text-[9px] font-black uppercase border ${u.role === 'DIRETORIA' ? 'bg-indigo-600 text-white border-indigo-700' :
                          ...
                          <div className="flex justify-between items-center border-b border-slate-200/60 pb-1">
                            ...
                            {isDirectory && (
                              <button
                                ...
                                className={`text-[7px] font-black uppercase px-2 py-0.5 rounded transition-all ${isAllSelected ? 'text-red-500 hover:bg-red-50' : 'text-blue-600 hover:bg-blue-50'
                                  }`}
                              >
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {SYSTEM_FEATURES.filter(f => f.group === group).map(feature => (
                              <button
                                ...
                                className={`flex items-center justify-between p-3 rounded-xl transition-all border ${editing.permissions?.includes(feature.id)
