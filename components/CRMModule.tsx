...
const KanbanCard: React.FC<{ 
    ...
    isFirstCol: boolean;
    isLastCol: boolean;
}> = ({ opp, isPriority, onClick, onMove, isFirstCol, isLastCol }) => {
    return (
        <div className={`p-4 rounded-2xl border transition-all cursor-pointer group relative ${isPriority ? 'bg-red-50/50 border-red-100 hover:border-red-300' : 'bg-white border-slate-100 hover:border-indigo-200 shadow-sm'}`}>
            {/* Header Card */}
            <div className="flex justify-between items-start mb-2" onClick={onClick}>
                <div className="flex-1 min-w-0 pr-2">
                    <h4 className="font-black text-slate-800 text-xs uppercase italic truncate">{opp.clientName}</h4>
                    {opp.companyName && <p className="text-[9px] font-bold text-slate-400 uppercase truncate">{opp.companyName}</p>}
                </div>
                {opp.instagramLink && (
                    <a href={opp.instagramLink} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="text-pink-600 hover:scale-110 transition-transform">
            ...
            {/* Badges Info */}
            <div className="space-y-1.5 mb-3" onClick={onClick}>
                {opp.attendant && (
                    <div className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tight">
                        <span>Closer: {opp.attendant.split(' ')[0]}</span>
                    </div>
                )}
                {opp.nextFollowUp && (
                    <div className={`flex items-center gap-1 text-[8px] font-black uppercase ${isPriority ? 'text-red-600' : 'text-slate-400'}`}>
                        <ICONS.History className="w-3 h-3" />
                        <span>{new Date(opp.nextFollowUp).toLocaleDateString()}</span>
                    </div>
                )}
            </div>

            {/* Move Controls (Overlay) */}
            <div className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-white to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex justify-between items-end px-2 pb-2">
                <button 
                    disabled={isFirstCol}
                    onClick={(e) => { e.stopPropagation(); onMove('PREV'); }}
                    className="p-1 bg-slate-100 rounded hover:bg-slate-200 text-slate-500 disabled:opacity-0"
                >
                ...
                <button 
                    ...
                    className="p-1 bg-indigo-100 rounded hover:bg-indigo-200 text-indigo-600 disabled:opacity-0"
                >
