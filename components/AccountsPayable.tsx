...
const AccountsPayableModule: React.FC = () => {
  ...
  const handleSyncExpenses = async () => {
    ...
    try {
      ...
      const erros = data?.totalErros || 0;

      if (erros > 0) {
        setToast({ msg: `Sync Parcial: ${novos} novos, ${atualizados} ok, ${erros} erros/avisos (API Bloqueada?). Ver Console.`, type: 'warning' });
        console.warn("Erros Sync:", data.erros);
      } else {
        setToast({ msg: `Sync OK! ${novos} novos, ${atualizados} atualizados.`, type: 'success' });
      }

      // Automatically trigger enrichment after sync
      await handleEnrichExpenses();
            ...
            <div>
              <nav className="relative z-0 inline-flex rounded-xl shadow-sm -space-x-px" aria-label="Pagination">
                <button
                  ...
                  className={`relative inline-flex items-center px-3 py-2 rounded-l-xl border border-slate-200 bg-white text-xs font-bold uppercase tracking-wider ${currentPage === 1 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-500 hover:bg-slate-50'
                    }`}
                >
                  Anterior
                </button>
                    ...
                    if (pageNum === 1 || pageNum === totalPages || (pageNum >= currentPage - 1 && pageNum <= currentPage + 1)) {
                      return (
                        <button
                          ...
                          className={`relative inline-flex items-center px-4 py-2 border text-xs font-bold ${currentPage === pageNum
                ...
                <button
                  ...
                  className={`relative inline-flex items-center px-3 py-2 rounded-r-xl border border-slate-200 bg-white text-xs font-bold uppercase tracking-wider ${currentPage === totalPages || totalPages === 0 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-500 hover:bg-slate-50'
                    }`}
                >
                  Próximo
                </button>
